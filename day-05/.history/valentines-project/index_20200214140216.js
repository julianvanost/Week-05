...
function validateAnswers(answer, panAnswers) {
  ...
  for (let i = 0; i < panAnswers.length; i++) {
    console.log(`${panel[i]} guessed: ${panAnswers[i]}`)
...
async function runMatchGame() {
  try {
    ...
    panel = await getUserInput('Please enter number of panel:', 'number')
    for (let i = 0; i < panel; i++) {
      panel.push(await getUserInput(`Please enter pnaelist #${i + 1}'s name:`, 'input'))
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      for (let j = 0; j < panel.length; j++) {
        panAnswers.push(await getUserInput(`Panelist #${j + 1} (${panel[j]}) Answer:`, 'password'))
      }
